...
function entityClient(entityName) {
  ...
  return {
    ...
    bulkCreate: (items) =>
      requestJson(`/entities/${entityName}/bulk`, { method: 'POST', body: items }),
    update: (id, data) =>
      requestJson(`/entities/${entityName}/${id}`, { method: 'PATCH', body: data }),
...
export const base44 = {
  ...
  integrations: {
    Core: {
      SendEmail: (payload) =>
        requestJson(`/integrations/Core/SendEmail`, { method: 'POST', body: payload }),
      ...
      ExtractDataFromUploadedFile: (payload) =>
        requestJson(`/integrations/Core/ExtractDataFromUploadedFile`, {
          method: 'POST',
          body: payload,
        }),
      InvokeLLM: (payload) =>
        requestJson(`/integrations/Core/InvokeLLM`, { method: 'POST', body: payload }),
      GenerateImage: (payload) =>
        requestJson(`/integrations/Core/GenerateImage`, { method: 'POST', body: payload }),
      CreateFileSignedUrl: (payload) =>
        requestJson(`/integrations/Core/CreateFileSignedUrl`, { method: 'POST', body: payload }),
      UploadPrivateFile: (payload) =>
        requestJson(`/integrations/Core/UploadPrivateFile`, { method: 'POST', body: payload }),
    },
  },
  auth: {
    ...
    signup: async ({ name, email, password }) => {
      const out = await requestJson(`/auth/signup`, {
        method: 'POST',
  ...
  payments: {
    ...
    generateSaveCardLink: async ({ memberId, guestId }) =>
      requestJson(`/payments/save-card-link`, {
        method: 'POST',
